...
def get_rcept_no(bgn_de, end_de) :
    ...
    info_df['비고'] = np.where(info_df.report_nm.str.contains('기재정정'),'정정',np.where(info_df.report_nm.str.contains('발행조건확정'),'발행','최초'))
...
def get_value_from_id(xml_text, table_id, value_id):
    table = re.findall('<TABLE-GROUP ACLASS="'+table_id+'".*?</TABLE-GROUP>', xml_text, re.DOTALL)[0].replace('TE', 'TD').replace('TU', 'TD')
    num = len(re.findall('"'+value_id+'".*?</TD>', table, re.DOTALL))
    if num>1 :
        value = []
        for i in range(num):
            value.append(re.findall('"'+value_id+'".*?</TD>', table, re.DOTALL)[i].replace('><','').split('>')[1].split('<')[0])
    else :
        value = re.findall('"'+value_id+'".*?</TD>', table, re.DOTALL)[0].replace('><','').split('>')[1].split('<')[0]
    return value
...
empty1, col1, col2 = st.columns([0.2, 0.5, 1.2])
with empty1:
    st.empty()
with col1:
    lottie = load_lottie()
